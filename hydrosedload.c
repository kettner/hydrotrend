...
//Tbar = Tmean - ( (lapserate[ep] * maxalt) / 3.0 );
    ...
    if (yr == syear[ep]){
        ...
        for (kk = 0; kk < nhypts[ep]; kk++){
            if (kk == 0)
                Tbar += (Tmean - (((hypselev[ep][kk] - hypselev[ep][0])) * lapserate[ep])) * (hypsarea[ep][kk] / totalarea[ep]);
            else
                Tbar += (Tmean - (((hypselev[ep][kk] - hypselev[ep][0])) * lapserate[ep])) * ((hypsarea[ep][kk] - hypsarea[ep][kk - 1]) / totalarea[ep]);
        }
    }
 ...
 *--------------------------------*/
        if (Qsbarformulaflag[ep] == 2){                       //BQART
            if (Tbar >= 2.0)
                Qsbartot[ep] = alpha9 * lithology[ep] * (1 - TE[ep]) * anthro[ep] * pow (A, alpha11) * pow (((Qbartotal[ep] * yTOs) / 1e9), alpha10) * (H * 0.001) * Tbar;
            
            if (Tbar < 2.0)
                Qsbartot[ep] = 2.0 * alpha9 * lithology[ep] * (1 - TE[ep]) * anthro[ep] * pow (A, alpha11) * pow (((Qbartotal[ep] * yTOs) / 1e9), alpha10) * (H * 0.001);
        }
        if (Qsbarformulaflag[ep] == 1)                        //ART
            Qsbartot[ep] = alpha3 * (1.0 - TE[ep]) * pow (A, alpha4) * pow (H, alpha5) * exp (k1 * Tbar);

        if (Qsbarformulaflag[ep] == 0)                       //QRT
 ...
 *--------------------------------------------*/
    trnfac = (rhowater * rhosed * trneff) / ((rhosed - rhowater) * tan (anglerep * degTOr));
 ...
 *--------------------------------------*/
     ...
     *-------------------------*/
                    ratio1 = Qsum[i][p] / Qbar[ep][p][eventcounter - eventsperyear];
                    if (setstartmeanQandQs == 3)
                        Qsoutlet[i][p] = Psi[i] * pow (((Qsbartotoutlet[ep][p] / Qsoutletdummy) * Qsbartot[ep]), C[yr - syear[ep]]) * pow (ratio1,C[yr - syear[ep]]);
                    if (setstartmeanQandQs == 4){
                        Qsoutlet[i][p] = Psi[i] * pow (((Qsbartotoutlet[ep][p] / Qsoutletdummy) * Qsbartot[ep]), C[yr - syear[ep]]) * Qsbarnew[ep] * pow (ratio1, C[yr - syear[ep]]);
     ...
     *-------------------------------------*/
                    Qboutlet[i][p] = trnfac * rslope[ep] * pow (Qsum[i][p], alphabed[ep]);
