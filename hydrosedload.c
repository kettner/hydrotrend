...
for (i=0; i < daysiy; i++) {
	...
	if (setstartmeanQandQs < 4){
		if ( (100 * glacierarea / totalarea) > 1.0 ){
		    Qspsi[i] = Psi[i] * pow((Qsumtot[i])/(Qbartotal[ep]), C[yr-syear[ep]]);
		    Qs[i] = Qspsi[i];
		}
		else{
			Qs[i] = Psi[i] * pow((Qsumtot[i])/(Qbartotal[ep]), C[yr-syear[ep]]);
			Qspsi[i] = Qs[i];
		}
	...
	if (setstartmeanQandQs == 4){
		 ...
		 *----------------------------------------------------------------*/
			Qspsi[i] =  (Psi[i] * Qsbarnew[ep] * pow((Qsumtot[i])/(Qbartotal[ep]), C[yr-syear[ep]]));
		 ...
		 *---------------------------------------------------------------------------*/			
			Qs[i] =  (Psi[i] * Qsbarnew[ep] * pow((Qsumtot[i])/(Qbartotal[ep]), C[yr-syear[ep]]));
			Qspsi[i] = Qs[i];	
		}
