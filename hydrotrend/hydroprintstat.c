 ...
 *--------------------*/
	...
	if (Qsbarformulaflag[ep] == 2){
		if (Tmean >=2.0){
			fprintf( fidstat, "Qsbar = alpha9 * Lithology * (1-TE) * Eh * pow(A,alpha11) * pow((Q*yTOs/1e9),alpha10) * H * Tbar\n" );
			...
			fprintf( fidstat, "\t alpha9, alpha10, alpha11 are global constants:\n");
			fprintf( fidstat, "\t alpha9 = %.2e\n",alpha9);
			fprintf( fidstat, "\t alpha10 = %.2e\n",alpha10);
			fprintf( fidstat, "\t alpha11 = %.2e\n",alpha11);		
			fprintf( fidstat, "Qsbar = %.2f (kg/s)\n",Qsbartot[ep]);
			fprintf( fidstat, "Glacier sediment flux is calculated seperately. The mean suspended sediment\n");
		...
		if (Tmean < 2.0){
			fprintf( fidstat, "Qsbar = 2 * alpha9 * Lithology * (1-TE) * Eh * pow(A,alpha11) * pow((Q*yTOs/1e9),alpha10) * H\n" );
			fprintf( fidstat, "\t A = %.2f; river basin area (km2)\n",A);
			fprintf( fidstat, "\t H = %.2f; maxalt = basin relief (km)\n\n",H/1000);
			fprintf( fidstat, "\t Lithology = %.2f (-)\n",lithology[ep]);
			fprintf( fidstat, "\t Eh = %.2f; Anthropogenic factor (-)\n",anthro[ep]);
	...
	if (Qsbarformulaflag[ep] == 1){
		fprintf( fidstat, "Qsbar =  alpha3 * pow(A,alpha4) * pow(H,alpha5) * exp(k1 * Tbar)\n" );
		fprintf( fidstat, "\t A = %.2f; river basin area (km2)\n",A);
		fprintf( fidstat, "\t H = %.2f; maxalt = basin relief (m)\n\n",H);
		fprintf( fidstat, "\t T = %.2f; Tbar = mean basin temp (C)\n",Tbar);
		fprintf( fidstat, "\t\t T =Tstart[ep](=%.2f) - ((lapserate[ep](=%f) * maxalt(=%.5f))/3.0\n\n",Tstart[ep],lapserate[ep],maxalt);
	...
	if (Qsbarformulaflag[ep] == 0){
		fprintf( fidstat, "Qsbar =  alpha6 * pow(Qbar,alpha7) * pow(H,alpha8) * exp(k2 * Tbar)\n" );
		fprintf( fidstat, "\t Qbar = %.2f; long-term average of Q (m3/s)\n",Qbartotal[ep]);
		fprintf( fidstat, "\t H = %.2f; maxalt = basin relief (m)\n\n",H);
		fprintf( fidstat, "\t T = %.2f; Tbar = mean basin temp (C)\n",Tbar);
		fprintf( fidstat, "\t\t T =Tstart[ep](=%.2f) - ((lapserate[ep](=%.5f) * maxalt(=%.2f))/3.0\n\n",Tstart[ep],lapserate[ep],maxalt);
	...
	fprintf( fidstat, "\t Qsbar = %.2f (kg/s); used long-term average of Qs (fractions of Qs(daily)/nr. of fractions)\n\n",Qsmean[ep]*Qsbarnew[ep]);
	...
	fprintf( fidstat, "\t Qbar = %.2f (m3/s); long-term average of Q, calculated by formula explained above\n\n",Qbartotal[ep]);
	fprintf( fidstat, "\t C\t= a random number from a distribution with mean E(C)\n" );
	fprintf( fidstat, "\t\t and standard deviation sigma-C, where:\n" );
	fprintf( fidstat, "\t\t E(C) = (1.4 - (0.025*T) + (0.00013*H) + (0.145*log10(Qsbar))\n" );
