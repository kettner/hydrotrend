...
int hydroglacial()
{
	 ...
	 *----------------------------------*/
		...
		for (ii=0; ii<daysiy; ii++) {
			...
			else Parea = 0.0; 				/* FLA is above the basin */
		...
		if( massavailable < 0.0 ) {
			...
			correctedVolumeglacierarea = ((0.8*MPglacial) + (Volumelast*1e6))/ 1e6;
			Gmass = -0.8*MPglacial;
			if ( setstartmeanQandQs == 4 ){		
				fprintf( stderr, "HydroGlacial WARNING: year = %d, ep =%d \n", yr, ep );
				fprintf( stderr, " \t Insufficient precipitation to grow glacier. \n" );
				...
				fprintf( fidlog, " \t Insufficient precipitation to grow glacier. \n" );
			...
			glacierareakmreset = pow ( (Volumeglacierarea/bethaglacier),(1/bethaexpo) );
		...
		if( setstartmeanQandQs == 3 ){	
			 ...
			 *--------------------------------------------------------------*/
				if ( (100 * glacierarea / totalarea) > 1.0 ){
					yieldQsbar = (Qsbartot[ep]*dTOs*365)/(1000*(totalarea/1e6));
					...
					yield = (pow(10,(1.7846 + (0.99126 * log10 ( (100 * (glacierarea / totalarea))) ))) + yielddifference) - yieldQsbar;					
		...
		if( setstartmeanQandQs == 0 ){	
			...
			if (GlacierMinput[ep] > 0.0 && (yr == (syear[ep] + nyears[ep])-1)){
				fractionglaciersediment[ep]=1.0-(fabs(GlacierMstorage[ep])/GlacierMinput[ep]);
			}		
			else fractionglaciersediment[ep] = 1.0;
		}
	}   /* endif glacial */
	return(err);
}  /* end of HydroGlacial */
